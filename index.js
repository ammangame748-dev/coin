...
// --- إعداد Multer لرفع الصور (تعريف عالمي لإصلاح الخطأ) ---
...
function getGuildConfig(guildId) {
    ...
    if (!db.guilds[guildId]) {
        db.guilds[guildId] = {
            ...
            logChannel: null,
            topChannel: null,
            cmdChannels: [], // القنوات المسموح فيها بالأوامر
            rankBanner: null,
            topBanner: null,
...
client.on('messageCreate', async message => {
    ...
    // التحقق من قنوات الأوامر
    const isCmdChannel = !config.cmdChannels || config.cmdChannels.length === 0 || config.cmdChannels.includes(message.channel.id);

    if (message.content === config.balanceCmd) {
        if (!isCmdChannel) return; // تجاهل الأمر إذا لم يكن في القناة المسموحة
...
client.on('voiceStateUpdate', (oldState, newState) => {
    ...
    const key = `${newState.guild.id}_${newState.member.id}`;
    if (!oldState.channelId && newState.channelId) voiceTracking.set(key, Date.now());
    else if (oldState.channelId && !newState.channelId) {
        const start = voiceTracking.get(key);
        if (start) {
            ...
            if (mins > 0) {
                ...
                if (!db.users[key]) db.users[key] = { points: 0, msgCount: 0, voiceMinutes: 0 };
                db.users[key].voiceMinutes = (db.users[key].voiceMinutes || 0) + mins;
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'give' || commandName === 'remove') {
        ...
        saveDB(db);
        interaction.reply({ content: `✅ تم التعديل بنجاح.`, ephemeral: true });
...
async function createTopEmbed(guildId) {
    ...
    desc += '```';
    const embed = new EmbedBuilder().setColor(config.embedColor).setTitle(`🏆 متصدرين ${config.coinName}`).setDescription(desc).setTimestamp().setFooter({ text: config.embedFooter });
...
<style>
    ...
    label { display: block; margin-top: 10px; color: #66fcf1; font-size: 0.8em; text-transform: uppercase; }
    ...
    .btn { background: #7289da; color: white; border: none; padding: 15px; margin-top: 20px; width: 100%; border-radius: 5px; cursor: pointer; font-family: 'Orbitron', sans-serif; font-weight: bold; transition: 0.3s; }
    .btn:hover { background: #66fcf1; color: #0b0c10; box-shadow: 0 0 20px #66fcf1; }
    .multi-select { height: 100px; }
</style>
`;

app.get('/', (req, res) => res.send(`<html><head>${neonCSS}</head><body><div class="container"><h1>🪙 CoinMaster Pro</h1><a href="/auth" class="btn">تسجيل دخول الإدارة 🚀</a></div></body></html>`));
...
app.get('/dashboard/:guildId', async (req, res) => {
    ...
    let chOpts = '';
    channels.forEach(c => { 
        const selected = config.cmdChannels?.includes(c.id) ? 'selected' : '';
        chOpts += `<option value="${c.id}" ${selected}>${c.name}</option>`; 
    });
            ...
            <div class="container">
                ...
                <form action="/save/${guild.id}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="section">
                        ...
                        <label>القنوات المسموح فيها بالأوامر (اختر واحدة أو أكثر):</label>
                        <select name="cmdChannels" class="multi-select" multiple>
                            ${chOpts}
                        </select>
                        <p style="font-size:0.7em; color:#888;">* اضغط Ctrl (أو Command) لاختيار أكثر من قناة.</p>
                    </div>

                    <div class="section">
                        <h2>📡 الرومات التلقائية</h2>
                        <label>روم اللوق (Logs):</label>
                        <select name="logChannel">
                            <option value="">-- تعطيل --</option>
                            ${chOpts.replace(/selected/g, '').replace(`value="${config.logChannel}"`, `value="${config.logChannel}" selected`)}
                        </select>
                        <label>روم التوب التلقائي:</label>
                        <select name="topChannel">
                            <option value="">-- تعطيل --</option>
                            ${chOpts.replace(/selected/g, '').replace(`value="${config.topChannel}"`, `value="${config.topChannel}" selected`)}
                        </select>
                    </div>
                    ...
                    <div class="section">
                        ...
                        <div class="grid" style="margin-top:10px;">
                            <div><label>هدف الصوت (دقائق):</label><input type="number" name="voiceGoal" value="${config.voiceGoal}"></div>
                            <div><label>جائزة الصوت:</label><input type="number" name="voiceReward" value="${config.voiceReward}"></div>
                        </div>
                    </div>

                    <div class="section">
                        <h2>🖼️ التصميم والبانرات</h2>
                        <label>نص الفوتر:</label><input type="text" name="embedFooter" value="${config.embedFooter}">
                        <label>لون الإمباد (Hex):</label><input type="text" name="embedColor" value="${config.embedColor}">
                        <div class="grid">
                            <div><label>بانر البطاقة:</label><input type="file" name="rankBannerFile"></div>
                            <div><label>بانر التوب:</label><input type="file" name="topBannerFile"></div>
                        </div>
                    </div>

                    <button type="submit" class="btn">حفظ وتطبيق 🚀</button>
                </form>
            </div>
        </body>
        </html>
...
app.post('/save/:guildId', upload.fields([{name:'rankBannerFile'}, {name:'topBannerFile'}]), (req, res) => {
    ...
    // معالجة اختيار القنوات المتعددة
    if (typeof updates.cmdChannels === 'string') updates.cmdChannels = [updates.cmdChannels];
    ...
    saveDB(db);
    res.send(`<html><head>${neonCSS}</head><body><div class="container"><h1>✅ تم الحفظ</h1><a href="/dashboard/${req.params.guildId}" class="btn">رجوع</a></div></body></html>`);
});
